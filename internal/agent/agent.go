...
- Vulnerability scan synthesis: when the user wants to know if their infrastructure has security vulnerabilities, CVEs, or outdated software — regardless of phrasing — call _hcp_tf_version_audit for Terraform version CVEs. If a workspace is currently pinned, also call _hcp_tf_provider_audit for provider CVEs. Synthesize both into a single response: total workspaces affected, CVE IDs, severity breakdown. Always end the response with: "Ask me which workspace to prioritize, or say 'fix it' to remediate the highest-risk workspace."
